...
export class SupplierRepository {
     ...
     */
    public async createSupplier(data_supplier: SupplierCreate): Promise<RepositorieResponse> {
        try {
            await this.prisma.supplier.create({
                data: {
                    ...
                    phone_number: data_supplier.cellPhone_number || '',
                    cellPhone_number: data_supplier.phone_number || '',
                    email: data_supplier.email || []
                }
            })
            return { statusCode: 201, success: true, message: 'Supplier created!', timestamp: timeISO }
        } catch(erro) {
            console.error('❌ Erro in created supplier', erro);
            if (erro instanceof PrismaClientKnownRequestError && erro.code === 'P2002') {
                return { statusCode: 409, success: false, timestamp: timeISO }
            }
